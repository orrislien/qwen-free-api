...
var ServiceConfig = class _ServiceConfig {
  ...
  constructor(options) {
    ...
    this.name = import_lodash3.default.defaultTo(name, "qwen-free-api");
...
async function createCompletion(model = MODEL_NAME, messages, ticket, retryCount = 0) {
  ...
  return (async () => {
    ...
    const session2 = await new Promise(
      (resolve, reject) => {
        const session3 = import_http2.default.connect("https://qianwen.biz.aliyun.com");
...
async function createCompletionStream(model = MODEL_NAME, messages, ticket, retryCount = 0) {
  ...
  return (async () => {
    ...
    session = await new Promise((resolve, reject) => {
      const session2 = import_http2.default.connect("https://qianwen.biz.aliyun.com");
...
function checkResult(result) {
  ...
  if (!import_lodash12.default.isBoolean(success) || success)
    return result.data;
      ...
      try {
        ...
        const text = (result.contents || []).reduce((str, part) => {
          ...
          if (contentType != "text" && contentType != "text2image")
            return str;
          if (role != "assistant" && !import_lodash12.default.isString(content))
            return str;
          return str + content;
        }, "");
  ...
  const parser = (0, import_eventsource_parser.createParser)((event) => {
    try {
      ...
      const text = (result.contents || []).reduce((str, part) => {
        ...
        if (contentType != "text" && contentType != "text2image")
          return str;
        if (role != "assistant" && !import_lodash12.default.isString(content2))
          return str;
        return str + content2;
      }, "");
      const exceptCharIndex = text.indexOf("\uFFFD");
...
async function checkFileUrl(fileUrl) {
  if (util_default.isBASE64Data(fileUrl))
    return;
  const result = await import_axios2.default.head(fileUrl, {
    timeout: 15e3,
    validateStatus: () => true
  });
...
async function uploadFile(fileUrl, ticket) {
  ...
  if (util_default.isBASE64Data(fileUrl)) {
    ...
    ({ data: fileData } = await import_axios2.default.get(fileUrl, {
      responseType: "arraybuffer",
      // 100M限制
  ...
  await import_axios2.default.request({
    method: "POST",
  ...
  if (isImage) {
    ...
    let result = await import_axios2.default.post(
    ...
    while (true) {
      result = await import_axios2.default.post(
...
var images_default = {
  ...
  post: {
    "/generations": async (request) => {
      request.validate("body.prompt", import_lodash14.default.isString).validate("headers.authorization", import_lodash14.default.isString);
      const tokens = chat_default.tokenSplit(request.headers.authorization);
      const token = import_lodash14.default.sample(tokens);
      const prompt = request.body.prompt;
      const responseFormat = import_lodash14.default.defaultTo(request.body.response_format, "url");
      const model = request.body.model;
